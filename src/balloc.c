...
static struct ext_list_head balloc_free_list;
...
{
	BUG_ON(ext_list_empty(&balloc_free_list) &&
				"Please, increase BALLOC_MAX_RANGES constant");

	struct ext_list_head *node = balloc_free_list.next;
...
{
	ext_list_add(&node->link.ll, &balloc_free_list);
}
...
{
	ext_list_init(&balloc_free_list);
