...
{
	struct page *page = __page_alloc(cache->pool_order);
...
void mem_cache_setup(struct mem_cache *cache, size_t size, size_t align)
{
	...
	ext_list_init(&cache->free_pools);
	ext_list_init(&cache->partial_pools);
	ext_list_init(&cache->busy_pools);
}

void mem_cache_shrink(struct mem_cache *cache)
{
	struct ext_list_head free_slabs;
	struct ext_list_head *head, *ptr;

	ext_list_init(&free_slabs);
	ext_list_splice(&cache->free_pools, &free_slabs);
...
{
	BUG_ON(!ext_list_empty(&cache->busy_pools));
	BUG_ON(!ext_list_empty(&cache->partial_pools));

	struct ext_list_head *head = &cache->free_pools;

	for (struct ext_list_head *ptr = head->next; ptr != head;) {
		struct mem_pool *pool = LIST_ENTRY(ptr, struct mem_pool, ll);
...
static void *mem_pool_alloc(struct mem_cache *cache, struct mem_pool *pool)
{
	...
	/* It's not a constant time at all, but performance wise
	 * bitmap is more cache efficient while it's small enough. */
...
{
	if (!ext_list_empty(&cache->partial_pools)) {
		struct ext_list_head *ptr = ext_list_first(&cache->partial_pools);
		struct mem_pool *pool = LIST_ENTRY(ptr, struct mem_pool, ll);

		void *data = mem_pool_alloc(cache, pool);

		if (!pool->free) {
			ext_list_del(&pool->ll);
			ext_list_add(&pool->ll, &cache->busy_pools);
		}
		return data;
	}

	if (!ext_list_empty(&cache->free_pools)) {
		struct ext_list_head *ptr = ext_list_first(&cache->free_pools);
		struct mem_pool *pool = LIST_ENTRY(ptr, struct mem_pool, ll);
		...
		if (pool->free != cache->obj_count) {
			ext_list_del(&pool->ll);
			ext_list_add(&pool->ll, &cache->partial_pools);
		}
		return data;
	}
...
void mem_cache_free(struct mem_cache *cache, void *ptr)
{
	...
	if (pool->free == cache->obj_count) {
		ext_list_del(&pool->ll);
		ext_list_add(&pool->ll, &cache->free_pools);
	}
}
