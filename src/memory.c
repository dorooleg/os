...
struct ext_list_head page_alloc_zones;
...
static void __page_alloc_zone_setup(uintptr_t zbegin, uintptr_t zend)
{
	...
	for (int i = 0; i != MAX_ORDER + 1; ++i)
		ext_list_init(&zone->order[i]);
	ext_list_add_tail(&zone->ll, &page_alloc_zones);
}

static struct page_alloc_zone *page_alloc_zone_find(uintptr_t idx)
{
	struct ext_list_head *head = &page_alloc_zones;
	struct ext_list_head *ptr;
...
static void __page_alloc_zone_free(uintptr_t zbegin, uintptr_t zend)
{
	...
	for (uintptr_t page = begin; page != end;) {
		...
		ext_list_add_tail(&ptr->ll, &zone->order[order]);
...
static void page_alloc_zone_dump(const struct page_alloc_zone *zone)
{
	...
	for (int order = MAX_ORDER; order >= 0; --order) {
		if (ext_list_empty(&zone->order[order]))
			continue;

		const unsigned long count = ext_list_size(&zone->order[order]);
...
void page_alloc_setup(void)
{
	...
	ext_list_init(&page_alloc_zones);
	...
	for (struct ext_list_head *p = head->next; p != head; p = p->next) {
...
static struct page *page_alloc_zone(struct page_alloc_zone *zone, int order)
{
	...
	while (ext_list_empty(&zone->order[current]) && current <= MAX_ORDER)
		++current;

	if (current > MAX_ORDER)
		return 0;

	BUG_ON(ext_list_empty(&zone->order[current]));

	struct page *page = LIST_ENTRY(ext_list_first(&zone->order[current]),
				struct page, ll);
	...
	while (current != order) {
		...
		ext_list_add(&buddy->ll, &zone->order[current]);
			...
			int order)
{
	...
	while (order < MAX_ORDER) {
		...
		ext_list_del(&buddy->ll);
		++order;
	}

	ext_list_add(&page->ll, &zone->order[order]);
