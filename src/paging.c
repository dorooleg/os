...
struct ext_list_head kmap_free_ranges[KMAP_ORDERS];
...
{
	struct ext_list_head *head = &kmap_free_ranges[order];
	struct ext_list_head *ptr = head->next;
...
static void kmap_free_range(struct kmap_range *range, size_t pages)
{
	if (range > kmap_ranges) {
		...
		if (!ext_list_empty(&prev->ll)) {
			ext_list_del(&prev->ll);
			pages += prev->pages;
			range = prev;
		}
	}
	...
	if (range + pages < kmap_ranges + KMAP_PAGES) {
		...
		if (!ext_list_empty(&next->ll)) {
			ext_list_del(&next->ll);
			pages += next->pages;
		}
	}

	(range + pages - 1)->pages = range->pages = pages;
	ext_list_add(&range->ll, &kmap_free_ranges[kmap_order(pages + 1) - 1]);
}
...
static struct kmap_range *kmap_alloc_range(size_t pages)
{
	...
	ext_list_del(&range->ll);
	ext_list_init(&range->ll);
	(range + pages - 1)->pages = range->pages = pages;
...
void kmap_setup(void)
{
	...
	for (int i = 0; i != KMAP_ORDERS; ++i)
		ext_list_init(&kmap_free_ranges[i]);
